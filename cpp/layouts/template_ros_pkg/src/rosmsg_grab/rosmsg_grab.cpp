...
int main(int argc, char* argv[])
{
  ...
  OdoRosHandler odo_ros_handler(ros_nh);
  ros::Subscriber sensor_gps = ros_nh.subscribe(ros_param.name.sensor + "/ublox/data"
                                              ,1000,&OdoRosHandler::gps_callback,&odo_ros_handler);
  ros::Subscriber sensor_imu = ros_nh.subscribe(ros_param.name.sensor + "/imu"
                                              ,1000,&OdoRosHandler::imu_callback,&odo_ros_handler);
  ros::Subscriber sensor_imu2 = ros_nh.subscribe(ros_param.name.sensor + "/imu2"
                                              ,1000,&OdoRosHandler::imu2_callback,&odo_ros_handler);
  ros::Subscriber sensor_steer = ros_nh.subscribe(ros_param.name.sensor + "/steer"
                                              ,1000,&OdoRosHandler::steer_callback,&odo_ros_handler);
  ros::Subscriber sensor_trigger = ros_nh.subscribe(ros_param.name.sensor + "/trigger"
                                              ,1000,&OdoRosHandler::trigger_callback,&odo_ros_handler);
  ros::Subscriber sensor_wssd = ros_nh.subscribe(ros_param.name.sensor + "/wssd"
                                              ,1000,&OdoRosHandler::wssd_callback,&odo_ros_handler);
  ros::Subscriber sensor_wssnd = ros_nh.subscribe(ros_param.name.sensor + "/wssnd"
                                              ,1000,&OdoRosHandler::wssnd_callback,&odo_ros_handler);
  ros::Subscriber sensor_adma = ros_nh.subscribe(ros_param.name.sensor + "/adma"
                                              ,1000,&OdoRosHandler::adma_callback,&odo_ros_handler);
...
int cmd_check(int argc, char* argv[])
{
  ...
  while((option = getopt(argc,argv,"hp:"))>=0)
  {
    switch (option)
    {
      case 'h': std::cout
                << "Usage: rosmsg_grab\n\n"
                << "Subscribes to Opel-CAN-data and distributes ROS-messages to OdoRosHandler-class. \n\n"
                << "Options: \n"
